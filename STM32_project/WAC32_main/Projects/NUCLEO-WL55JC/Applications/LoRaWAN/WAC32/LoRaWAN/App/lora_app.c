...
/**
  * @brief  SCD30 calibration callback function
  * @param  context ptr of LED context
  */
static void scd30_calibration(void *context);
...
void LoRaWAN_Init(void)
{
  ...
  UTIL_SEQ_RegTask((1 << CFG_SEQ_Task_CO2Calibration), UTIL_SEQ_RFU, scd30_calibration);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  switch (GPIO_Pin)
  {
    case  BUTTON_SW1_PIN:
      ...
      break;
    case  BUTTON_SW2_PIN:
    	UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_CO2Calibration), CFG_SEQ_Prio_0);
      break;
    case  BUTTON_SW3_PIN:
      break;
    default:
...
static void scd30_calibration(void *context)
{
	scd30_set_forced_recalibration(CO2_CALIBRATION);
	BSP_I2C2_Init();
	rgb_lcd_clear();
	rgb_lcd_setCursor(0,0);
	rgb_lcd_noBlinkLED();
	rgb_lcd_setColorWhite();
	rgb_lcd_print("CO2 Recalibrate");
	rgb_lcd_setCursor(0,1);
	rgb_lcd_print("at %d ppm...", CO2_CALIBRATION);
	HAL_Delay(20000);
	BSP_I2C2_DeInit();
}
...
static void SendTxData(void)
{
  ...
  pm10		  = (uint16_t)SensorData.pm10;
  pm25		  = (uint16_t)SensorData.pm25;
  ...
  AppData.Buffer[i++] = (uint8_t)((pm25 >> 8) & 0xFF);
  AppData.Buffer[i++] = (uint8_t)(pm25 & 0xFF);
  AppData.Buffer[i++] = (uint8_t)((pm10 >> 8) & 0xFF);
    AppData.Buffer[i++] = (uint8_t)(pm10 & 0xFF);
...
static void OnSensorTimerEvent(void *context)
{
  ...
  if((co2>min_co2)&&(co2<max_co2)){
	  rgb_lcd_blinkLED_medium();
	  rgb_lcd_setRGB(63, 63, 0);
  }
  else if(co2>max_co2){
	  rgb_lcd_blinkLED();
	  rgb_lcd_setRGB(127, 0, 0);
  }
  else{
	  rgb_lcd_noBlinkLED();
  ...
  APP_LOG(TS_ON, VLEVEL_M, " Sensor data : | CO2: %d ppm | T: %d °C | Hum: %d % | PM10 : %d µg | PM2.5 : %d µg |\n\r", co2, temperature, humidity, pm10, pm25);
