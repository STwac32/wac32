...
uint32_t i2c_send_byteS_rgb(unsigned char* dta, unsigned char len) {
	return BSP_I2C2_Send(RGB_ADDRESS, dta, len);
}

void getUart(uint8_t *PData, uint16_t Size, uint8_t Error)
{
	static int char_pos = 0;
	static char string[UART_MAX_BUFFER];
	const char delim[1] = " ";
	char *token;

	if((char)*PData=='\r'){		// Check if return is pressed
		token = strtok(string, delim);		// Get the first word
		if(strcmp(token, "min") == 0){
			token = strtok(NULL, delim); 	// Get the second word
			min_co2 = atoi(token);
			APP_LOG(TS_OFF, VLEVEL_M, "\nNew minimum co2 entered : %d ppm \n\r", min_co2);
		}
		if(strcmp(token, "max") == 0){
			token = strtok(NULL, delim); 	// Get the second word
			max_co2 = atoi(token);
			APP_LOG(TS_OFF, VLEVEL_M, "\nNew maximum co2 entered : %d ppm \n\r", max_co2);
		}
		if(strcmp(token, "get") == 0){
			APP_LOG(TS_OFF, VLEVEL_M, "\nActual limits values for CO2 : min = %d | max = %d \n\r", min_co2, max_co2);
		}
		char_pos = 0;
		memset(string, 0, sizeof(string));	// Reset buffer
	}
	else if(char_pos<UART_MAX_BUFFER){
		string[char_pos] = (char)*PData;
		APP_LOG(TS_OFF, VLEVEL_M, "%c", string[char_pos]);
		char_pos++;
	}
	else
		char_pos = 0;
...
void SystemApp_Init(void)
{
  /* USER CODE BEGIN SystemApp_Init_1 */
	/* Set limits value for blinking LCD */
	min_co2 = 800;
	max_co2 = 1000;
  ...
  /*Initialize the terminal for receiving*/
  UTIL_ADV_TRACE_StartRxProcess(*getUart);
